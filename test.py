...
class Trie:
    ...
    def to_template_thing(self):
        subs = ",".join([x.to_template_thing() for x in self.d.values()])
...
with open("greetings.hxx", "w") as greetings_file:
    ...
    for s in greetings:
        x.add(s)
    x.populate_counts()
    greetings_file.write(template.replace("%%%", x.to_template_thing()).replace("$$$", "greetings"))
...
with open("fortunes.hxx", "w") as fortunes_file:
    ...
    for s in fortunes:
        x.add(s)
    x.populate_counts()
    fortunes_file.write(template.replace("%%%", x.to_template_thing()).replace("$$$", "fortunes"))
